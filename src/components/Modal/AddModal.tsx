	...
	return (
		<Modal show={showModal} title={`Add ${title}`} close={closeModal}>
			<Form onSubmit={(e) => submit(e, total)}>
				<DP selectedDate={item.date} setDate={(date: Date) => updateItem('date', date, null)} label={`${title}`} />
				{showModal && (
					<select
				...
				{item.categories.length ? (
					<ul className='list-unstyled'>
						{item.categories.map((i, idx) => (
							<li key={i.amount + idx} className='d-flex justify-content-between align-items-center px-3'>
