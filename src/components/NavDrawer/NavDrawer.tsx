...
const NavDrawer: React.FC<Props> = ({ isShowing, selectedLink, setSelectedLink, toggleDrawer }) => {
	...
	return (
		<>
			<div className={`Backdrop ${isShowing ? 'Show' : 'NoShow'}`} onClick={toggleDrawer} />
			<div
				className={`NavDrawer-Cont text-center pt-4 border-end border-info border-2 ${isShowing ? 'Show' : 'NoShow'}`}
			>
				{links.map((i) => (
